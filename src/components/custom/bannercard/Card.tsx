...
export const Card = ({ title, imgUrl, video }: ICardProps) => {
  ...
  useEffect(() => {
    if (isInView) setMaxSrollY(scrollY.get());
  }, [isInView]);
    ...
    <motion.section
      ...
      <div className="relative h-full">
        ...
        <div className="absolute  -bottom-12 left-16 right-16  bg-opacity-75  px-8 flex justify-between items-center">
          <span
            id="badge"
            className="ml px-3 py-1 bg-neutral-500 rounded-2xl text-sm">
            2024
          </span>
          <h1
            className={`${
              video ? "text-black" : "text-white"
            } font-caps text-[5rem] sm:text-[10rem] pb-16`}>
            {title}{" "}
          </h1>
          <span
            id="badge"
            className="px-3 py-1 bg-neutral-500 rounded-2xl text-sm">
            {video ? "Video" : "Photo"}
          </span>
        </div>
      </div>
